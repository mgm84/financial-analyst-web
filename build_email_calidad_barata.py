...
S_BODY = "font-family:Arial,Helvetica,sans-serif;background:#f2f2f2;margin:0;padding:24px;color:#222;"
S_CONTENEDOR = "max-width:960px;margin:0 auto;background:#ffffff;border:1px solid #ddd;"
S_TD_BASE = "padding:6px 6px;text-align:left;font-size:11px;word-wrap:break-word;"

S_CABECERA_TITULO = S_TD_BASE + "background:#1a3c5e;color:#ffffff;font-size:18px;font-weight:bold;padding:16px;"
S_CABECERA_SUB = S_TD_BASE + "background:#1a3c5e;color:#cdddec;font-size:12px;padding:0 16px 14px 16px;"
S_RESUMEN_CAJA = S_TD_BASE + "background:#eef4fa;padding:14px 16px;font-size:13px;line-height:1.5;"
S_TITULO_SECCION = S_TD_BASE + "background:#2d5f8a;color:#ffffff;font-size:14px;font-weight:bold;padding:10px 12px;"
S_TITULO_SUBSECCION = S_TD_BASE + "background:#dbe7f2;color:#1a3c5e;font-size:12.5px;font-weight:bold;padding:7px 12px;"
S_TH = S_TD_BASE + "background:#f4f6f8;color:#555;font-size:9px;text-transform:uppercase;border-bottom:2px solid #ddd;"
...
S_LEYENDA_CAJA = "background:#f8f8f8;padding:12px 16px;font-size:11.5px;line-height:1.6;border-top:1px solid #eee;"
S_PIE = "background:#f4f4f4;color:#999;font-size:10.5px;padding:12px 16px;text-align:center;"
...
S_PILL_NUEVO = S_PILL_BASE + "background:#fde8cc;color:#a15c00;"
S_PILL_SOLAPA = S_PILL_BASE + "background:#f6dede;color:#a3282e;"
S_PILL_DETERIORO = S_PILL_BASE + "background:#eee;color:#666;"
...
def fila_general(fila, nombres, con_sector, fondo):
    ...
    fondo_style = f"background:{fondo};" if fondo != FILA_BLANCA else ""
    ...
    celda_sector = f'<td style="{td_plano}">{fila["sector"]}</td>' if con_sector else ""

    return f"""
    <tr>
      <td style="{td_rank}">{fila['puesto_general']}</td>
      <td style="{td_valor}">{ticker}<span style="{S_SPAN_NOMBRE}">{nombre}</span></td>
      {celda_sector}
      <td style="{td_num}">{fmt_precio(fila['precio'])}</td>
      <td style="{td_num}color:{color_num(fila['variacion_1d'])};">{fmt_pct(fila['variacion_1d'])}</td>
      <td style="{td_num}color:{C_BAJADA};">{fmt_pct(fila['vs_max_52s'])}</td>
      <td style="{td_num}color:{C_BAJADA};">{fmt_pct(fila['vs_max_9a'])}</td>
      <td style="{td_metrica}">{fila['metrica']}</td>
      <td style="{td_num}">{fmt_ratio(fila['ratio_hoy'])}</td>
      <td style="{td_num}">{fmt_ratio(fila['mediana_historica'])}</td>
      <td style="{td_num}color:{color_percentil(fila['percentil'])};font-weight:bold;">{fila['trimestres_mas_barato']}/{fila['trimestres_totales']}</td>
      <td style="{td_num}color:{color_barato(fila['cuanto_mas_barata'])};font-weight:bold;">{fmt_pct(fila['cuanto_mas_barata'])}</td>
      <td style="{td_plano}width:100px;">{''.join(alertas)}</td>
    </tr>"""
...
def tabla_sector(sector, filas, nombres):
    ...
    return f"""
  <table><tr><td style="{S_TITULO_SUBSECCION}">{sector}</td></tr></table>
  <table style="border-collapse:collapse;width:100%;">
    <tr>
      <th style="{S_TH}">Nº/75</th><th style="{S_TH}">Valor</th>
      <th style="{S_TH}text-align:right;">Cierre</th><th style="{S_TH}text-align:right;">1D</th>
      <th style="{S_TH}text-align:right;">vs. máx 52s</th><th style="{S_TH}text-align:right;">vs. máx hist. (9a)</th>
      <th style="{S_TH}">Métrica</th><th style="{S_TH}text-align:right;">Ratio hoy</th>
      <th style="{S_TH}text-align:right;">Mediana hist.</th><th style="{S_TH}text-align:right;">Trim. más barato</th>
      <th style="{S_TH}text-align:right;">Cuánto más barata</th><th style="{S_TH}width:100px;"></th>
    </tr>
    {filas_html}
  </table>"""
...
<div style="{S_CONTENEDOR}">
  ...
  <table style="border-collapse:collapse;width:100%;">
    <tr><td style="{S_CABECERA_TITULO}">📊 Calidad + Barata — Monitor diario</td></tr>
    <tr><td style="{S_CABECERA_SUB}">{ranking_hoy['fecha']}</td></tr>
  </table>

  <table style="border-collapse:collapse;width:100%;"><tr><td style="{S_RESUMEN_CAJA}">
  ...
  <table style="border-collapse:collapse;width:100%;"><tr><td style="{S_TITULO_SECCION}">🏆 Ranking general — Top 10 más baratas de las {ranking_hoy['universo_calculable']}</td></tr></table>
  <table style="border-collapse:collapse;width:100%;">
    <tr>
      <th style="{S_TH}">Nº/75</th><th style="{S_TH}">Valor</th><th style="{S_TH}">Sector</th>
      <th style="{S_TH}text-align:right;">Cierre</th><th style="{S_TH}text-align:right;">1D</th>
      <th style="{S_TH}text-align:right;">vs. máx 52s</th><th style="{S_TH}text-align:right;">vs. máx hist. (9a)</th>
      <th style="{S_TH}">Métrica</th><th style="{S_TH}text-align:right;">Ratio hoy</th>
      <th style="{S_TH}text-align:right;">Mediana hist.</th><th style="{S_TH}text-align:right;">Trim. más barato</th>
      <th style="{S_TH}text-align:right;">Cuánto más barata</th><th style="{S_TH}width:100px;">Alertas</th>
    </tr>
    {filas_generales}
  </table>

  <table style="border-collapse:collapse;width:100%;"><tr><td style="{S_TITULO_SECCION}">📂 Ranking por sector</td></tr></table>
  {tablas_sector}

  <table style="border-collapse:collapse;width:100%;"><tr><td style="{S_LEYENDA_CAJA}">
  ...
  <table style="border-collapse:collapse;width:100%;"><tr><td style="{S_PIE}">
